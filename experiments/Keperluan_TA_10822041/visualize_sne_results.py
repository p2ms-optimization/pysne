...
def parse_args():
    ...
    # Construct description showing available problem IDs/keys
...
def plot_1d_results(prob, roots, save_path, no_show=False):
    ...
    # Generate high resolution line
    n_pts = 1000
    x = np.linspace(x_min, x_max, n_pts)
    
    # SNE has a system of equations. For 1D, we plot the single equation curve.
    eq = prob.equations[0]
    try:
    ...
    ax.set_title("System Equations and Roots", fontsize=14)
    ax.set_xlabel("x1", fontsize=12)
    ax.set_ylabel("f(x1)", fontsize=12)
    ...
    print(f"Saved 1D equation contour plot to: {save_path}")
...
def plot_2d_results(prob, roots, save_path, no_show=False):
    ...
    # SNE has a system of equations, we plot contours at level 0 for each equation.
    # Typically in 2D SNE we have exactly 2 equations.
...
def plot_3d_results(prob, roots, save_path_prefix, no_show=False):
    ...
    # Plot 3D Scatter (Thresholded Peak/Root visualization)
    n_points = 20
    ...
    # Roots correspond to peaks (fitness near 1.0)
    ...
    # Overlay found roots in 3D scatter
    if len(roots) > 0:
