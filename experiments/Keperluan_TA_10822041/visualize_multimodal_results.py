...
def plot_2d_results(prob, maxima, minima, save_path, no_show=False):
    ...
    # Highlight Maxima on 3D surface
    if len(maxima) > 0:
        z_maxima = []
        for pt in maxima:
    ...
    # Highlight Minima on 3D surface
    if len(minima) > 0:
        z_minima = []
        for pt in minima:
            z_val = prob.g_func(pt)
...
def plot_3d_results(prob, maxima, minima, save_path_prefix, no_show=False):
    ...
    # Overlay found Maxima in 3D scatter
    if len(maxima) > 0:
    ...
    # Overlay found Minima in 3D scatter
    if len(minima) > 0:
    ...
    z_range = z_max - z_min
    slice_tolerance = 0.15 * z_range  # Max distance from slice to display point
    ...
    for idx, z_val in enumerate(z_slices):
        ...
        # Plot maxima that are close to this slice
        if len(maxima) > 0:
            ...
            if len(pts_near) > 0:
                ...
                for pt in pts_near:
                    axes[idx].text(pt[0] + (x_max - x_min)*0.02, pt[1] + (y_max - y_min)*0.02, 
                                   f"Max (Z={pt[2]:.2f})", color='darkred', fontsize=8, fontweight='bold', zorder=6)
                    
        # Plot minima that are close to this slice
        if len(minima) > 0:
            ...
            if len(pts_near) > 0:
                ...
                for pt in pts_near:
                    axes[idx].text(pt[0] + (x_max - x_min)*0.02, pt[1] + (y_max - y_min)*0.02, 
                                   f"Min (Z={pt[2]:.2f})", color='darkblue', fontsize=8, fontweight='bold', zorder=6)
...
def main():
    ...
    # Try converting input argument to int if possible, else keep string key
    try:
