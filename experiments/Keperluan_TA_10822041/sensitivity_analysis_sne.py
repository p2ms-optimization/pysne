"""
sensitivity_analysis_sne.py
===========================
Modul analisis sensitivitas parameter khusus untuk problem SNE (Systems of Non-linear Equations) pada PySNE.

Menguji pengaruh variasi parameter kunci terhadap Discovery Rate (DR)
pada 7 problem benchmark SNE.

Cara menjalankan eksperimen:
    python sensitivity_analysis_sne.py --experiment gamma
    python sensitivity_analysis_sne.py --experiment k_cluster
    python sensitivity_analysis_sne.py --all --export csv
"""
...
# Konfigurasi benchmark SNE (7 problem)
SNE_BENCHMARK_CONFIG = [
    (1, "Problem 1 (2D, 6 akar)",   6),
    (2, "Problem 2 (2D, 12 akar)", 12),
    (3, "Problem 3 (6D, 2 akar)",   2),
    (4, "Problem 4 (1D, 9 akar)",   9),
    (5, "Problem 5 (5D, 3 akar)",   3),
    (6, "Problem 6 (8D, 16 akar)", 16),
]

# Definisi eksperimen sensitivitas parameter untuk SNE
EXPERIMENTS = {
    "gamma": {
        "description": "Pengaruh parameter cut-off gamma terhadap Discovery Rate pada SNE",
        "param_key": "gamma",
        "values": [0.05, 0.1, 0.2, 0.3, 0.5],
        "value_labels": ["0.05", "0.1", "0.2", "0.3", "0.5"],
        "target": "sne",
        "note": "gamma mengontrol threshold keakuratan residual minimum untuk clustering.",
    },
    "k_cluster": {
        "description": "Pengaruh jumlah iterasi clustering (k_cluster) terhadap Discovery Rate dan waktu pada SNE",
        "param_key": "k_cluster",
    ...
    "delta": {
        "description": "Pengaruh parameter resolusi jarak (delta) terhadap Discovery Rate pada SNE",
        "param_key": "delta",
    ...
    "sdoa_m": {
        "description": "Pengaruh ukuran populasi SDOA (sdoa_m) per cluster terhadap Discovery Rate pada SNE",
        "param_key": "sdoa_m",
    ...
    "m_cluster": {
        "description": "Pengaruh jumlah titik awal clustering (m_cluster) terhadap Discovery Rate pada SNE",
        "param_key": "m_cluster",
    ...
    "num_check_points": {
        "description": "Pengaruh parameter num_check_points (multi-point check) terhadap Discovery Rate pada SNE",
...
def _print_experiment_header(name, cfg):
    print("\n" + "=" * 80)
    print(f"EKSPERIMEN SENSITIVITAS SNE: {name.upper()}")
...
def _print_summary_table(experiment_name, cfg, results):
    ...
    print(f"\n{'='*80}")
    print(f"RINGKASAN DISCOVERY RATE SNE - {experiment_name.upper()}")
    print(f"{'='*80}")
    ...
    for label in param_labels:
        ...
        avg = np.mean(vals) if vals else 0.0
        time_row += f"  {avg:.2f}s".rjust(col_w + 2)
    print(time_row)
    print(f"{'='*80}\n")
...
def generate_latex_table(results, experiment_name, cfg, caption=None, label=None):
    ...
    if caption is None:
        caption = f"Analisis Sensitivitas Parameter ${param_key}$ pada Problem SNE: Discovery Rate"
    if label is None:
        label = f"tab:sensitivity_sne_{experiment_name}"
...
def run_all_experiments(n_runs=1, benchmark_ids=None, export_format=None, output_dir=".", verbose=True):
    """Jalankan semua eksperimen sensitivitas."""
    all_results = {}

    for exp_name in EXPERIMENTS:
        try:
            results = run_sensitivity_experiment(exp_name, n_runs=n_runs, benchmark_ids=benchmark_ids, verbose=verbose)
...
def main():
    parser = argparse.ArgumentParser(description="Analisis Sensitivitas Parameter PySNE Khusus SNE")
    ...
    parser.add_argument("--problems", "-p", type=str, default=None, help="Daftar ID problem benchmark SNE yang dipisahkan koma, misal: 1,2,3")
    parser.add_argument("--export", choices=["csv"], default=None, help="Format export hasil.")
    parser.add_argument("--output-dir", default="sensitivity_results_sne", help="Direktori output.")
    ...
    if args.list:
        print("\nEksperimen sensitivitas SNE yang tersedia:\n")
    ...
    if args.problems:
        try:
            benchmark_ids = [int(x.strip()) for x in args.problems.split(",")]
        except ValueError:
            print("[ERROR] Format parameter --problems salah. Gunakan angka dipisahkan koma, contoh: 1,2,3")
            sys.exit(1)
