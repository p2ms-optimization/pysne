...
from pysne.problems.benchmarks_gecco import get_gecco_problems
from pysne.solver import solve_system
from pysne.problems.base import BaseProblem



def test_gecco_pipeline_execution():
    """
    Tes sederhana (smoke test) untuk memastikan pipeline gecco 
    berjalan dari awal hingga akhir menggunakan OOP benchmarks.
    """
    # 1. Ambil daftar problem
    problems = get_gecco_problems()
    ...
    # Pastikan solve_gecco menerima (objective_function, domain, params)
    # roots = solve_gecco(prob2.g_func, domain, test_params)
...
if __name__ == "__main__":
    test_gecco_pipeline_execution()

def test_gecco_problem(
    problem_id: int,
    verbose: bool = True
    ) -> Dict[str, Any]:
    result = {
    ...
    try:
        ...
        if verbose:
            print(f"\n[STEP 1] Loading Problem {problem_id} (gecco)")
        ...
        class MinimizedProblem:
            def __init__(self, original_prob):
                ...
                self.equations = None
                self.problem_type = getattr(original_prob, 'problem_type', 'gecco')
        ...
        if verbose:
            ...
            print("\n[STATUS]: SUCCESS! Gecco Multimodal analysis complete.")
...
def test_all_gecco_problems(problem_ids: list = None, show_details: bool = False):
    if problem_ids is None:
        problem_ids = [1, 2, 3, 4, 5, 6]
        
    print("="*60)
    print("PYSNE GECCO MULTIMODAL COMPREHENSIVE TEST")
    print("="*60 + "\n")
    ...
    for problem_id in problem_ids:
        results[problem_id] = test_gecco_problem(problem_id, verbose=show_details)
        
    total_time = time.time() - total_start

    print("\n" + "="*60)
    print("SUMMARY GECCO MULTIMODAL RESULTS")
    print("="*60)
...
if __name__ == "__main__":
    print("\nPySNE Gecco Multimodal Test Suite")
    print("Testing objective function optimizers\n")
    
    if len(sys.argv) > 1:
        command = sys.argv[1].lower()
        if command == "all":
            test_all_gecco_problems(show_details=False)
        elif command == "all-verbose":
            test_all_gecco_problems(show_details=True)
        elif command.isdigit():
            test_gecco_problem(int(command), verbose=True)
    else:
        test_all_gecco_problems(show_details=False)
